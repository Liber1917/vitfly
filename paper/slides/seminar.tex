...
\ \ ——\ 参数量最小，反而是最慢的！
\item \textbf{根因}：SS2D编码器的四方向选择性扫描\\
每次扫描独立执行，四次累积
\end{itemize}
\end{block}
...
\footnotesize
B+和E在BC下只有3次碰撞，\\
\pos{蒸馏后降到1次，甚至超越教师的2次！}
\end{block}

\column{0.48\textwidth}
\begin{alertblock}{但为什么A和C蒸馏无效？}
\begin{itemize}\footnotesize
\item A (SS2D编码器)：扫描展平2D $\to$ 1D
\item C (CNN编码器)：卷积保留2D空间结构
\item \textbf{设计对照实验}：E vs E-SSM\\
将E的CNN编码器换成SSM，其余不变
\end{itemize}
\end{alertblock}

\begin{block}{E-SSM对照结果}
\centering\footnotesize
\begin{tabular}{lcc}
\toprule
...
\end{block}
\vspace{0.2cm}
\begin{block}{G\_basic的启示}
\footnotesize
CNN+MLP (0.49M, 0.74ms)\\
仅比E多1.8次碰撞，参数少78\%，快9.6倍\\
$\to$ \textbf{SSM时序头在T=1下边际贡献有限}
\end{block}
\end{columns}
\end{frame}
